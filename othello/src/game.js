...
export default class Game extends React.Component {
  ...
  handleClick(i) {
    ...
    if (
      this.calculateWinner(current.xNumbers, current.oNumbers) ||
      current.squares[i]
    ) {
      return;
    }

    const changedSquares = this.flipSquares(
      current.squares,
      i,
      this.state.xIsNext
    );
    ...
    const xNumbers = changedSquares.reduce((acc, current) => {
      return current === "X" ? acc + 1 : acc;
    }, 0);
    const oNumbers = changedSquares.reduce((acc, current) => {
      return current === "O" ? acc + 1 : acc;
    }, 0);

    let shouldTurnColor =
      this.checkAvailableMoves(!this.state.xIsNext, changedSquares).length > 0
        ? !this.state.xIsNext
        : this.state.xIsNext;
  ...
  render() {
    ...
    let availableMoves = this.checkAvailableMoves(
      current.xWasNext,
      current.squares
    );
    let availableMovesOpposite = this.checkAvailableMoves(
      !current.xWasNext,
      current.squares
    );
    ...
    let status = winner
      ? winner === "XO"
        ? "It's a draw"
        : "The winner is " + (winner === "X" ? "white" : "black")
      : [
          this.state.xIsNext ? "Whites turn" : "Blacks turn",
          " with ",
