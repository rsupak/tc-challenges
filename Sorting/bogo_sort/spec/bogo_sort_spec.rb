...
describe "#bogo_sort" do
  ...
  it "sorts the array from least to greatest" do
    expect( bogo_sort([6,2,5,3,4]) ).to eq [2,3,4,5,6]
    expect( bogo_sort(shuffled_array) ).to eq sorted_array
  end

  it "handles zero appropriately" do
    expect( bogo_sort([6,2,5,3,4,0]) ).to start_with 0
  end

  it "handles negative numbers appropriately" do
    expect( bogo_sort([6,2,-11, 5,3,4]) ).to start_with -11
  end
end 
